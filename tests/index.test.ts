...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['KATHAN_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Kathan) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['KATHAN_LOG'] = 'debug';
      const client = new Kathan({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['KATHAN_LOG'] = 'not a log level';
      const client = new Kathan({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'KATHAN_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Kathan({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['KATHAN_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Kathan({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['KATHAN_BASE_URL'] = 'https://example.com/from_env';
      const client = new Kathan({ apiKey: 'My API Key' });
    ...
    test('empty env variable', () => {
      ...
      const client = new Kathan({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://kathan-backend.getalchemystai.com');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Kathan({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['KATHAN_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Kathan({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Kathan({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Kathan({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Kathan({ apiKey: 'My API Key', fetch: testFetch });
